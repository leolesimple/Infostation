...
function updateHorairesv2(stopId = 58566) {
    ...
    const urlArret = "./json/arrets-train-u.json";
    const urlMission = "./json/dessertes/train-u.json";
...
function prochainTrain() {
    ...
    monitoredStopVisits.forEach((stopVisit, index) => {
            ...
            if (operatorRef.startsWith("SNCF_ACCES_CLOUD")) {
                ...
                if (lineref === "STIF:Line::C01741:" && destName !== station) {
                    mission = monitoredVehicleJourney.JourneyNote[0].value;

                    if (station === "La Défense (Grande Arche)" || station === "La Verrière") {
                        if (retard === 'delayed') {
