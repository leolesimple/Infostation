...
if (localStorage.getItem("stop_name") !== null && localStorage.getItem("stop_name") !== undefined) {
    gare = localStorage.getItem("stop_name");
}

function updateHorairesv2(stopId = 58566) {
    const url = 'https://prim.iledefrance-mobilites.fr/marketplace/stop-monitoring';
    const urlArret = "./json/arrets-trainl.json";
    const urlMission = "./json/dessertes/train-l.json";
...
function prochainTrain() {
    ...
    if (monitoredStopVisits.length === 0) {
        ...
        document.getElementById('station').textContent = gare;
        return;
    }
    ...
    monitoredStopVisits.forEach((stopVisit, index) => {
            ...
            if (operatorRef.startsWith("SNCF_ACCES_CLOUD")) {
                ...
                let h3quai;
                const DepartureStatus = monitoredVehicleJourney.MonitoredCall.DepartureStatus;
                ...
                const destinationDisplay = monitoredVehicleJourney.MonitoredCall.DestinationDisplay[0].value;
                ...
                let ml = '';
                const now = new Date();
                console.info(DepartureStatus + " " + destName)
                ...
                if (destinationDisplay.startsWith("Gare Saint") || destinationDisplay === "Gare Saint-Lazare") {
                    destName = "Paris Saint-Lazare"
                }
                ...
                if (lineref === "STIF:Line::C01740:" && operatorRef.startsWith("SNCF_ACCES_CLOUD") && destName !== station) {
                    mission = monitoredVehicleJourney.JourneyNote[0].value;

                    if (station === "Paris Saint-Lazare" || station === "Cergy-le-Haut" || station === "Maisons-Laffitte" || station === "Versailles Rive Droite" || station === "Saint-Nom-la-Bretèche Forêt de Marly" || (station === "Nanterre Université") && (mission === "POPI" || mission === "POPA")) {
                        if (retard === 'delayed') {
                            ...
                            heure_passage = '<span class="big-text blink" style="color:orangered">Supprimé.</span>';
                    ...
                    if (arrivalCountdown > -2 && !rechercheCorrespondance(station, mission)) {
                        ...
                        console.info("2" + DepartureStatus + " " + destName)
...
function onStationClick(stopId, stopName) {
    ...
    localStorage.setItem("stop_name", stopName);
    prochainTrain()
}
