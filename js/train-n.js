...
function updateHorairesv2(stopId = 58566) {
    ...
    const urlArret = "./json/arrets-train-n.json";
    const urlMission = "./json/dessertes/train-n.json";
...
function prochainTrain() {
    ...
    monitoredStopVisits.forEach((stopVisit, index) => {
            ...
            if (operatorRef.startsWith("SNCF_ACCES_CLOUD")) {
                ...
                } else if (station.startsWith("Gare Mont")) {
                    station = station.replace("Gare Mont", "Paris Mont");
                }

                if (destinationDisplay.startsWith("Gare Mont") || destinationDisplay === "Gare Montparnasse") {
                    destName = "Paris Montparnasse"
                }
                ...
                } else if (destName.startsWith("Gare Mont")) {
                    destName = destName.replace("Gare Mont", "Paris Mont");
                }


                if (lineref === "STIF:Line::C01736:" && operatorRef.startsWith("SNCF_ACCES_CLOUD") && destName !== station) {
                    mission = monitoredVehicleJourney.JourneyNote[0].value;

                    if (station === "Paris Montparnasse" || station === "Dreux" || station === "Rambouillet" || station === "Mantes la Jolie" || (station === "Plaisir Grignon" && mission === "POGI" || mission === "PAGO")) {
                        if (retard === 'delayed') {
