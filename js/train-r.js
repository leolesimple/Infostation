...
function updateHorairesv2(stopId = 58566) {
    ...
    const urlArret = "./json/arrets-train-r.json";
    const urlMission = "./json/dessertes/train-r.json";
...
function prochainTrain() {
    ...
    monitoredStopVisits.forEach((stopVisit, index) => {
            ...
            if (operatorRef.startsWith("SNCF_ACCES_CLOUD")) {
                ...
                if (station.startsWith("Gare de Lyon")) {
                    station = station.replace("Gare de Lyon", "Paris Gare de Lyon");
                ...
                if (destName.startsWith("Gare de Lyon")) {
                    destName = destName.replace("Gare de Lyon", "Paris Gare de Lyon");
                } else if (destName.startsWith("Gare de la")) {
                ...
                if (lineref === "STIF:Line::C01731:" && destName !== station) {
                    mission = monitoredVehicleJourney.JourneyNote[0].value;

                    if (station === "Melun" || station === "Montargis" || station === "Montereau" || station === "Montargis" || station === "Paris Gare de Lyon") {
                        if (retard === 'delayed') {
