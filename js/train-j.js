...
function updateHorairesv2(stopId = 58566) {
    ...
    const urlArret = "./json/arrets-trainj.json";
    const urlMission = "./json/dessertes/train-j.json";
...
function prochainTrain() {
    ...
    monitoredStopVisits.forEach((stopVisit, index) => {
            ...
            if (operatorRef.startsWith("SNCF_ACCES_CLOUD")) {
                ...
                if (lineref === "STIF:Line::C01739:" && operatorRef.startsWith("SNCF_ACCES_CLOUD") && destName !== station) {
                    mission = monitoredVehicleJourney.JourneyNote[0].value;

                    if (station === "Paris Saint-Lazare" || station === "Mantes la Jolie" || station === "Boissy l'Aillerie" || station === "Gisors" || station === "Ermont Eaubonne" || station === "Vernon - Giverny") {
                        if (retard === 'delayed') {
